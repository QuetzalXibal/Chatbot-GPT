...
def cambiar_transparencia(value):
    alpha = int(value) / 100
    ventana.wm_attributes("-alpha", alpha)

def ventana_transparencia():
    transparencia_ventana = tk.Toplevel(ventana)
    transparencia_ventana.title("Ajustar transparencia")
    transparencia_ventana.geometry("300x100")
    transparencia_slider = ttk.Scale(transparencia_ventana, from_=10, to=100, orient='horizontal', command=cambiar_transparencia)
    transparencia_slider.set(100)
    transparencia_slider.pack(expand=True, padx=20, pady=20)
...
transparencia_menu.add_command(label="Ajustar transparencia", command=ventana_transparencia)
